// Cloudflare Worker for SB's vacature-tracker push subscriptions and status updates.
// Same pattern as Arjen's own vacature-tracker Worker — this file is deliberately
// generic (all repo/owner details come from env vars below), so it's an exact copy;
// only the Worker's deployed name and its env vars differ between the two trackers.
//
// The browser can't write to the GitHub repo directly (no credentials, and it
// shouldn't have any), so this Worker is the thin authenticated backend that
// receives subscribe/unsubscribe/status-update calls from the site and persists
...
//   GITHUB_REPO    = "SB-vacature-tracker"
//   GITHUB_BRANCH  = "main"
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // /update-status: called from the site when a vacancy's "Sollicitatie
      // status" dropdown changes. Persists straight into data.json's active
      // array, matched by link. Deliberately does NOT move "Afgewezen"/"Niet
      // interessant" items to the archive itself — that happens the next time
      // the scheduled check runs, keeping this endpoint a simple, fast write.
...
// i.e. someone else — usually the scheduled-check task — wrote in between).
async function updateJsonFile(env, filePath, mutate) {
  ...
  const ghHeaders = {
    ...
    "User-Agent": "sb-vacature-tracker-worker",
