...
// Triggered manually via workflow_dispatch — either directly on GitHub, or
// automatically by the Cowork scheduled task that checks this tracker twice
// a week (Monday + Thursday), which clicks "Run workflow" after every
// successful run, whether or not the active vacancy list changed. This is
// intentional: the notification is the signal that the check actually ran
// and the site reflects fresh data, not just a "something's new" alert.
const fs = require("fs");
...
const SITE_URL = process.env.SITE_URL || "https://arjen-rave.github.io/SB-vacature-tracker/";
...
(async () => {
  ...
  for (const sub of subscriptions) {
    try {
      ...
      // prioritize delivery even while the device is idle/Doze. TTL caps how
      // long the push service should keep retrying if the device is briefly
      // offline.
