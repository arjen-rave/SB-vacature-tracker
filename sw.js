...
// — safe to embed client-side. This is a separate key pair from Arjen's own
// vacature-tracker, generated specifically for this site.
const VAPID_PUBLIC_KEY = "BJ9LsXBpKr_THPpoA6ngAYdfvNDXxi-xdhc-UWpERQeC83zmEVivkVxLo-7arWVEm9MGLrnDdKwf6TW7NnRF-nU";
const WORKER_URL = "https://sb-vacature-tracker-subscribe.arjen-ravestein.workers.dev";
...
// out and a fresh copy in. Keep this in sync with the version footer in index.html.
const APP_VERSION = "1.1";
const CACHE = `sb-vacature-tracker-v${APP_VERSION}`;
const ASSETS = [
  "./",
  "index.html",
  "manifest.json",
...
// actual "cache update" mechanism: bump APP_VERSION, the old cache gets
// removed here on the next visit, so nothing stale lingers. Also claim
// existing open clients so an already-open tab/PWA gets handed to the new
// worker immediately (see the controllerchange listener in index.html, which
// reloads the page the moment that handover actually happens).
...
// and the tracker must never show a stale copy of the vacancy list.
